...
def _candidate_asset_roots():
    ...
    env_root = os.getenv('LATS_ASSET_ROOT')
    if env_root:
    ...
    sibling_root = os.path.join(workspace_root, 'Lifespan_Age_Trans')
...
def _resolve_asset_path(relative_path):
    for root in _candidate_asset_roots():
        ...
        if os.path.isfile(candidate):
            return candidate
    return os.path.join(MODULE_ROOT, relative_path)


resnet_file_path = _resolve_asset_path(os.path.join('deeplab_model', 'R-101-GN-WS.pth.tar'))
deeplab_file_path = _resolve_asset_path(os.path.join('deeplab_model', 'deeplab_model.pth'))
predictor_file_path = _resolve_asset_path(os.path.join('util', 'shape_predictor_68_face_landmarks.dat'))
model_fname = deeplab_file_path
...
class preprocessInTheWildImage():
    def __init__(self, out_size=256):
        ...
        try:
            ...
            raise ImportError(
                'dlib is required for --in_the_wild mode. '
                'Install dlib or run without --in_the_wild.'
            ) from exc
